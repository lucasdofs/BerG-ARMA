...
if(phi != 0 && theta != 0){
  ...
  H<- function(par) berg_cov(par,y,X,n,p,q,link="log")
...
forecast_bergARMA <- function(alpha,beta,phi,theta,nu,y,X_new,X_for,h,link = "log"){
...
#Tests
alpha=0.1;beta=c(0.5);phi=c(0.2);theta=c(0.2); nu=2
n=50
...
X_new=cbind(1,X)
#X_new=cbind(rep(1,n))
fitt=bergARMA(c(alpha,beta),phi,theta,nu,y,X_new,link="log")
fitt$par
diag(fitt$cov)^(1/2)
fitt$AIC
...
#testing with data TALVEZ ROLE
y=scan("Weiss_emergencies_96.txt")
ts.plot(y)
acf2(y)
X=cbind(rep(1,n))
ufit=glm.bg(y~1)
...
fit=bergARMA(beta=0.5,phi=0.5,theta=0,nu=1.8,y,X,link="log")
fit$par
fit$AIC
acf2(fit$resid)
